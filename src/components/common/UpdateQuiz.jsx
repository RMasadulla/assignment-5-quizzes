...
export default function UpdateQuiz({ ques, onNewquesAdded }) {
  const { axiosInstance } = useAxios();

  // Ensure ques and ques.options are defined
  const [quizTitle, setQuizTitle] = useState(ques?.question || "");
  const [options, setOptions] = useState(
    ques?.options?.map((option, index) => ({
      text: option,
      isCorrect: ques.correctAnswer === option,
    })) || [
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axiosInstance.patch(
        `${ADMIN_URL}/quess/${ques?.id}`,
        quizData
      );
      onNewquesAdded(response.data?.data);
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-foreground">Update Quiz</h2>
